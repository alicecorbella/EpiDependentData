...
lP <- function(y, p, pi=THETAvec[2]){
  switch(p, 
         ...
         dunif(toTh.mulpi(y[2:3])[2], 0, 0.1, log=T)+
           ((y[3])-(2*log(1+exp(y[3]))+log(1+exp(y[2])))),
         dunif(exp(y[4])/(exp(y[4])+1), 0,1, log=T)+((y[4])-(2*log(1+exp(y[4])))),
         dunif(exp(y[5])/(exp(y[5])+1), 0,1, log=T)+((y[5])-(2*log(1+exp(y[5])))))
}
...
adptMHsingle <- function(start, sdini, nsmpl, thin, np, stp.dpt, 
  ...
  IT         <- nsmpl*thin
  chain      <- matrix(NA, nsmpl, 5)
  old        <- start
  naccept    <- matrix(0, ncol = 5, nrow = IT)
  acclike    <- matrix(NA, ncol = 5, nrow = IT)
  it         <- 1
  oldTh      <- toTh(old) 
