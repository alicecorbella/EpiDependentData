...
s              <- 76  #as.numeric(task_id_string)
load(paste(maind,"/input/BIGsim",s,"GS.Robj", sep=""))
...
dpt   <- adptMHsingle(start=start1, sdini=c(0.05, 0.05, 1), nsmpl=DPT.nsmpl, thin=DPT.thin, 
                      ...
                      thetaconst=THETACONST,MCWM=F) 
Sys.time()
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))

ch1 <- runMHsingle (start=start1, sd=dpt[[1]], nsmpl=SMP.nsmpl, thin=SMP.thin, np=npset,MCWM=F)
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))
Sys.time()
ch2 <- runMHsingle (start=start2, sd=dpt[[1]], nsmpl=SMP.nsmpl, thin=SMP.thin, np=npset,MCWM=F)
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))
Sys.time()
ch3 <- runMHsingle (start=start3, sd=dpt[[1]], nsmpl=SMP.nsmpl, thin=SMP.thin, np=npset,MCWM=F)
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))
Sys.time()
...
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))
Sys.time()
ch1BLK <- runMHblocked(start=start1, vcscaled=dptBLK[[1]]*vc, nsmpl=SMP.nsmplB, thin=SMP.thinB, np=npset,MCWM=F)
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))
Sys.time()
ch2BLK <- runMHblocked(start=start2, vcscaled=dptBLK[[1]]*vc, nsmpl=SMP.nsmplB, thin=SMP.thinB, np=npset,MCWM=F)
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))
Sys.time()
ch3BLK <- runMHblocked(start=start3, vcscaled=dptBLK[[1]]*vc, nsmpl=SMP.nsmplB, thin=SMP.thinB, np=npset,MCWM=F)
save.image(file=paste(maind,"/outputDET/BIGsim",s,"GS.Robj", sep=""))
Sys.time()
